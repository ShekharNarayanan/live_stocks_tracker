...
def insert_portfolios_row(
    conn: sqlite3.Connection, email: str, cap_size: str, ticker: str
) -> bool:
    """
    ...
    cur = conn.execute(
        "SELECT 1 FROM portfolios WHERE email=? AND ticker=?",
        (email, ticker),
    )
    ...
    conn.execute(
        "INSERT INTO portfolios (email, cap_size, ticker) VALUES (?,?,?)",
...
def fetch_portfolio_from_db(
    email: str, cap_size: str, DB_PATH: pathlib.Path
    ...
    if not symbols:
        return [], pd.DataFrame(
            columns=["Symbol", "Change", "RSI14", "AvgVol30d", "Price", "Price_30d_ago"]
        )
